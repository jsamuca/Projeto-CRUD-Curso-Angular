...
private id: string
  ...
  ngOnInit(): void {
    this.subscriptions.add(this.activatedRoute.params.subscribe(({id, nome})=>{
      if(id!==undefined){
        this.id=id

    this.subscriptions.add(this.cursoService.consultar(nome).subscribe(curso=>{
      if(curso.length){
        const c = curso.find(c=>c.codigo===parseInt(this.id,10))
        this.cursoForm.setValue(c)
        this.cursoForm.get("dataInicio").setValue(this.changeDateFormat(new Date(c.dataInicio)))
  ...
  incluir(){
    this.subscriptions.add(this.cursoService.incluir(this.cursoForm.value).subscribe(async response=>{
  ...
  alterar(){
    this.subscriptions.add(this.cursoService.alterar(this.cursoForm.value).subscribe(async response=>{
        await this.router.navigate(['/curso']);
